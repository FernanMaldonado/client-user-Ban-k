...
export const Step2Confirm = () => {
    const { transferData, setStep, executeTransfer, loadingAccion } = useTransferStore();
        ...
        try {
            ...
            if (authRes && authRes.data && authRes.data.success) {
                ...
                if (transferRes.success) {
                    toast.success(transferRes.message);
                } else {
                    toast.error(transferRes.message);
                }
            } else {
    ...
    return (
        <div className="animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <h2 className="text-2xl font-black text-slate-900 tracking-tighter mb-6">Confirmar Transferencia</h2>
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-[2rem] p-6 mb-8">
                <div className="space-y-4">
                    <div className="flex justify-between items-center border-b border-slate-200/60 pb-3">
                        ...
                        <span className="font-black text-cyan-950">{transferData.cuentaOrigen}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-slate-200/60 pb-3">
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Destino</span>
                        <span className="font-black text-cyan-950">{transferData.cuentaDestino}</span>
                    </div>
                    <div className="flex justify-between items-center pt-2">
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Monto</span>
                        <span className="font-black text-2xl text-emerald-600">Q{Number(transferData.monto).toFixed(2)}</span>
                    </div>
                </div>
            </div>
