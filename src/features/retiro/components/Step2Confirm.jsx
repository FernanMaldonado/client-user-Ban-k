...
export const Step2Confirm = () => {
    const { retiroData, setStep, executeRetiro, loadingAccion } = useRetiroStore();
    ...
    const handleConfirm = async (e) => {
        ...
        try {
            ...
            if (authRes && authRes.data && authRes.data.success) {
                // Password is correct, proceed with retiro
                const retiroRes = await executeRetiro();
                if (retiroRes.success) {
                    toast.success(retiroRes.message);
                } else {
                    toast.error(retiroRes.message);
                }
            } else {
    ...
    return (
        <div className="animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <h2 className="text-2xl font-black text-slate-900 tracking-tighter mb-6">Confirmar Retiro</h2>
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-[2rem] p-6 mb-8">
                <div className="space-y-4">
                    <div className="flex justify-between items-center border-b border-slate-200/60 pb-3">
                        ...
                        <span className="font-black text-cyan-950">{retiroData.account_number}</span>
                    </div>
                    <div className="flex justify-between items-center pt-2">
                        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Monto</span>
                        <span className="font-black text-2xl text-emerald-600">Q{Number(retiroData.monto).toFixed(2)}</span>
                    </div>
                </div>
            </div>
