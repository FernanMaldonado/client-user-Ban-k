...
import { DepositoWizard } from '../components/DepositoWizard';
...
export const DepositosPage = () => {
    ...
    return (
        <div className="flex flex-col h-full animate-in fade-in zoom-in duration-700">
            ...
            <header className="flex justify-between items-center mb-10 pb-6 border-b border-slate-200">
                <div>
                    <h1 className="text-4xl font-black text-slate-800 tracking-tighter">
                        Módulo de <span className="text-cyan-900">Depósitos</span>
                    </h1>
                    <p className="text-slate-500 font-medium text-sm mt-2">
                        Deposita fondos de forma rápida y segura entre tus cuentas.
                    </p>
                </div>
            ...
            {/* Modal Historial */}
            <Modal isOpen={isHistorialOpen} onClose={() => setIsHistorialOpen(false)} title="Depósitos Realizados">
                {loadingHistorial ? (
                    <p className="text-slate-400 text-center py-8">Cargando historial...</p>
                ) : historial.length === 0 ? (
                    <p className="text-slate-400 text-center py-8">No hay depósitos registrados.</p>
                ) : (
                    <div className="space-y-4 max-h-96 overflow-y-auto pr-2 custom-scrollbar">
                        {historial.map(t => (
                            <div key={t._id} className="bg-slate-800 p-4 rounded-2xl border border-white/5 flex justify-between items-center">
                                <div>
                                    <p className="text-sm font-bold text-white uppercase tracking-wider">A: {t.account_number || t.numeroCuentaDestino}</p>
