...
export const Step1Deposito = () => {
    const { misCuentas, depositoData, setDepositoData, setStep } = useDepositoStore();

    const handleNext = (e) => {
        e.preventDefault();
        if (depositoData.cuentaId && depositoData.account_number && depositoData.monto) {
            setStep(2);
        }
    };
    ...
    return (
        <form onSubmit={handleNext} className="animate-in fade-in slide-in-from-right-4 duration-500">
            <h2 className="text-2xl font-black text-slate-900 tracking-tighter mb-6">Detalles del Depósito</h2>
            ...
            <div className="space-y-6">
                ...
                <div className="group space-y-2">
                    ...
                    <div className="relative">
                        <select
                            ...
                            value={depositoData.cuentaId}
                            onChange={(e) => setDepositoData({ cuentaId: e.target.value })}
                            required
                        >
                ...
                <div className="group space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-5 transition-colors group-focus-within:text-cyan-950">
                        Cuenta Destino (Tus Cuentas)
                    </label>
                    <div className="relative">
                        <select
                            className="w-full px-7 py-4 bg-slate-50 border border-slate-100 rounded-[2rem] text-slate-800 font-bold focus:outline-none focus:ring-4 focus:ring-cyan-950/5 focus:border-cyan-950 focus:bg-white transition-all appearance-none cursor-pointer"
                            value={depositoData.account_number}
                            onChange={(e) => setDepositoData({ account_number: e.target.value })}
                            required
                        >
                            <option value="" disabled>Selecciona tu cuenta destino</option>
                            {misCuentas.map(cuenta => (
                ...
                <div className="group space-y-2">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            onChange={(e) => setDepositoData({ monto: e.target.value })}
                            required
                        />
                    </div>
                </div>
