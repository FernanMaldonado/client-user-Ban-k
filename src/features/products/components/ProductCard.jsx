...
export const ProductCard = ({ product, onComprarClick, onCardClick }) => {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={(e) => {
          e.stopPropagation();
          if (isAvailable && isActive) onComprarClick(product.id);
        }}
        className={`w-full ${
      ...
      >
        {!isActive ? 'Producto inactivo' : isAvailable ? 'Comprar' : 'Agotado'}
      </button>
    </div>
  );
};
